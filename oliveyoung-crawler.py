...
try:
    ...
    try:
        ...
        # 예외 처리: main_image_urls 배열이 비어있는 경우
        if not main_image_urls:
            raise ValueError("Error: 메인 이미지 URL 배열(main_image_urls)이 비어있습니다.")
        ...
        # 예외 처리: category_id = 0 인 경우
        if category_id == 0:
            raise ValueError("Error: category_id가 0입니다. 유효한 카테고리 ID를 찾지 못했습니다.")

        # product_id
        product_id = create_product_id(product_name)

        # 예외 처리: product_id = 0 인 경우
        if product_id == 0:
            # `product_id = 0`은 일반적으로 이미 존재하는 제품명일 때 발생시키므로,
            # 해당 로직을 분리하거나, 기존 로직에 맞게 조정이 필요할 수 있습니다.
            # 여기서는 '새로운 제품을 생성해야 하는데 0이 나왔을 경우'를 가정하고 예외를 발생시킵니다.
            # 만약 '이미 존재하는 제품명'이라는 의미라면 아래 `else` 블록에서 처리하는 것이 맞습니다.
            # 하지만 요청에 따라 여기서는 `product_id = 0`이면 예외를 발생시키도록 합니다.
            raise ValueError("Error: product_id가 0입니다. 제품 ID 생성에 실패했거나 이미 존재하는 제품명입니다.")

        ### 소라 ###
        # 상품정보 제공 고시 수집+저장
        product_detail_info, product_detail_info_id = get_product_dtailinfo_provided(driver,
        ...
        # 예외 처리: detail_image_urls 배열이 비어있는 경우
        if not detail_image_urls:
            raise ValueError("Error: 상세 이미지 URL 배열(detail_image_urls)이 비어있습니다.")

        ### 소라 ###

        # product_id > 0 경우 : 새로운 product_name 인 경우에만 클로링을 계속 이어나가도록 하자
        # (주의: 위에서 이미 product_id == 0일 때 예외가 발생하도록 수정되었으므로, 이 if 문은
        # 기존 로직을 유지하면서 `product_id > 0`일 때만 DB 업데이트를 진행하도록 합니다.)
        if product_id > 0:
            # 상품 SQL 파일 업데이트
            ...
            # 기존 로직: product_id가 0일 때 출력되던 메시지.
            # (위에서 이미 예외가 발생했으므로 이 블록은 실행되지 않을 가능성이 높습니다.
            #  `create_product_id` 함수의 내부 로직에 따라 조절 필요)
        ...
        # 예외 처리: product_options 배열이 비어있는 경우
        if not product_options:
            raise ValueError("Error: 상품 옵션 배열(product_options)이 비어있습니다.")
        ...
        if product_options:
            ...
            # (위에서 이미 예외가 발생했으므로 이 블록은 실행되지 않습니다.)
            print("⚠ 수집된 옵션이 없습니다.")
        ### 민석 끝 ###
